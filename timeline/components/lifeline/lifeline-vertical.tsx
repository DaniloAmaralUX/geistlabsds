...
function RailTick({ present = false }: { present?: boolean }) {
  if (present) {
    return (
      <span
        aria-hidden="true"
        className="block h-2.5 w-2.5 rounded-full bg-blue-500 ring-2 ring-white dark:ring-black"
      />
    )
  }
    ...
    >
      ...
      >
        <div className={`${GRID_CLASS} items-center`}>
          <p
            className={cn(
              "text-right text-[11px] font-medium leading-4 tabular-nums text-zinc-500 transition-colors duration-300 dark:text-zinc-600",
              marker.present && "text-blue-600 dark:text-blue-400",
            )}
          >
            {age}
          </p>
          ...
          <div className="flex items-center justify-center">
            <RailTick present={marker.present} />
          </div>

          <p
            className={cn(
              "text-[15px] font-medium leading-5 tabular-nums text-zinc-500 transition-colors duration-300 dark:text-zinc-400",
              marker.present && "text-black dark:text-white",
            )}
          >
        ...
        {hasContent && (
          <div className={`${GRID_CLASS} mt-6`}>
            ...
            <div
              className={cn(
                "min-w-0 text-zinc-500 transition-colors duration-300 dark:text-zinc-400",
                marker.present && "text-black dark:text-zinc-100",
              )}
            >
