...
// LAB / DESIGN: the left fade starts under the sticky labels, not at the
// stage edge, so a column parked right after them (the present) reads whole.
const LEFT_EXIT_FADE_ZONE = 170
const LEFT_EXIT_FADE_ZONE_COARSE = 100
...
interface LifelineScrollOptions {
  mode?: LifelineMode
  /**
   * LAB / DESIGN: marker index the intro settles on, pinned right after
   * the sticky labels. Defaults to the end of the rail.
   */
  settleIndex?: number
...
export function useLifelineScroll(
  ...
  const settleTranslate = useRef(0)
  const settleIndexRef = useRef(options.settleIndex)
  ...
  settleIndexRef.current = options.settleIndex
  ...
  const updateFades = useCallback(() => {
    ...
    markerRefs.current.forEach((marker) => {
      ...
      // LAB / DESIGN: a column only dims once it slides under the pinned
      // labels; from there it fades out over `leftFadeZone` px.
      const shieldRight = LIFELINE_STICKY_LEFT + LIFELINE_STICKY_SHIELD_WIDTH
      if (markerLeft < shieldRight) {
        opacity = (markerLeft - shieldRight + leftFadeZone) / leftFadeZone
      }
  ...
  const measureLayout = useCallback(() => {
    ...
    // LAB / DESIGN: where the intro parks. The present marker, pinned right
    // after the sticky labels — or the end of the rail when no index is given.
    const settleIndex = settleIndexRef.current
    const settleMarker =
      settleIndex === undefined ? null : markerRefs.current[settleIndex]
    settleTranslate.current = settleMarker
      ? clamp(
          startInset.current - LIFELINE_STICKY_LEFT + settleMarker.offsetLeft,
          0,
          max,
        )
      : max
  ...
  useLayoutEffect(() => {
    ...
    if (!initialized.current) {
      ...
      translatePx.current = introSkippedRef.current ? settleTranslate.current : 0
  ...
  useEffect(() => {
    ...
    const step = (now: number) => {
      ...
      applyTranslateRef.current(progress * settleTranslate.current)
      ...
      sectionRef.current?.style.setProperty("--lifeline-intro-progress", "1")
      applyTranslateRef.current(settleTranslate.current)
      introScrollId.current = 0
    }
