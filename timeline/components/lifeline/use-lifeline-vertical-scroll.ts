...
interface LifelineVerticalScrollOptions {
  /**
   * LAB / DESIGN: entry index the intro settles on. Defaults to the end.
   */
  settleIndex?: number
  /**
...
export function useLifelineVerticalScroll(
  ...
  const settleScrollRef = useRef(0)
  ...
  const measureLayout = useCallback(() => {
    ...
    // LAB / DESIGN: where the intro parks — the present entry, a little
    // below the top of the scroller — or the end when no index is given.
    const settleIndex = settleIndexRef.current
    const target =
      settleIndex === undefined ? null : entryRefs.current[settleIndex]
    if (target) {
      const isDocument = scrollParent === document.scrollingElement
      const parentTop = isDocument
        ? 0
        : scrollParent.getBoundingClientRect().top
      // The stage clears the fixed nav with padding; land below it.
      const parentPadding =
        parseFloat(window.getComputedStyle(scrollParent).paddingTop) || 0
      const top =
        ...
        24
      settleScrollRef.current = clamp(top, 0, max)
    } else {
      settleScrollRef.current = max
    }
  ...
  useLayoutEffect(() => {
    ...
    if (!initialized.current) {
      scrollParent.scrollTop =
        introSkippedRef.current && !isEmbedRef.current
  ...
  useEffect(() => {
    ...
    const step = (now: number) => {
      ...
      if (max > 0) {
        applyScroll(progress * settleScrollRef.current)
      }
      ...
      if (max > 0) applyScroll(settleScrollRef.current)
      introScrollId.current = 0
    }
