...
export interface LifelineMarker {
  ...
  /**
   * LAB / DESIGN: largura fixa da coluna no desktop, em px. Sem ela a
   * largura vem da distância até o próximo marco (`getMarkerWidth`).
   */
  width?: number
  /**
   * LAB / DESIGN: marca o presente. Ponto azul na linha, texto em destaque
   * e, com `settleAt`, o lugar em que a abertura para.
   */
  present?: boolean
}
...
export interface LifelineProps {
  ...
  mode?: LifelineMode
  /**
   * LAB / DESIGN: índice do marco em que a abertura para (e onde a trilha
   * abre sob `prefers-reduced-motion`). Sem ele, o fim da trilha, como no
   * Lifeline original.
   */
  settleAt?: number
}